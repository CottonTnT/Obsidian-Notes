...
> 类的静态函数也是一个普通函数，每个类只有一份, 没有一个隐藏的参数来传入一个caller的指针，故该函数中不能访问类的非静态成员，非静态成员函数等等
...
- 成员初始化列表*使用初始化的方式*来为数据成员指定初值，而构造函数的函数体是*通过赋值的方式*来给数据成员指定初值。
- 也就是说，成员初始化列表是在数据成员定义的同时赋初值，但是构造函数的函数体是采用先定义后赋值的方式来做。这样的区别就造成了，在有些场景下，是必须要使用成员初始化列表。
