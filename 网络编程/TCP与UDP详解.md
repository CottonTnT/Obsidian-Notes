...
### 1.1.1 **3**  为什么需要 TCP 协议？ TCP 工作在哪一层？
...
源地址和目地址的字段（32 位）是在 IP 头部中，作用是通过 IP 协议发送报文给对方主机。
...
### 3.3.3  为什么每次建立 TCP 连接时，初始化的序列号都要求不一样呢？
...
# 4 cat /proc/sys/net/ipv4/tcp_syn_retries
5
...
# 5 cat /proc/sys/net/ipv4/tcp_synack_retries
5
...
### 5.1.2 **11**  什么是 SYN 攻击？如何避免 SYN 攻击？
