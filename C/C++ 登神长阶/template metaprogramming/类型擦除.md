...
private:
    ...
    static std::true_type test(
        decltype(conv(std::declval<const U&>() == std::declval<const U&>())), decltype(!conv(std::declval<const U&>() == std::declval<const U&>())));
...
public:
    ...
    virtual auto clone() -> functor_bridge* const = 0;
    virtual auto invoke(Args... args) -> R const = 0;
    virtual auto equals(const functor_bridge*) -> bool const = 0;
};
...
public:
    ...
    {
        auto tmp = function { std::forward<F>(rhs) };
        swap(*this, tmp);
        return *this;
    }
