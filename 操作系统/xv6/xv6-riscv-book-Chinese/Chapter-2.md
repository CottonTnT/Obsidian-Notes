...
当RISC-V计算机开机时，它会初始化自己，并运行一个存储在*只读存储器*中的**boot loader**。**Boot loader***将xv6内核加载到内存*中。然后，在*机器模式*下，CPU从 `_entry`（kernel/entry.S:6）开始执行xv6。RISC-V*在禁用分页硬件的情况下启动：虚拟地址直接映射到物理地址*。
