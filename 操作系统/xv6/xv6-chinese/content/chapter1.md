...
**每个进程都有用户栈和内核栈**（*`p->kstack`*）。当进程运行用户指令时，只有其用户栈被使用，其*内核栈则是空的*。然而当进程（通过系统调用或中断）进入内核时，内**核代码就在进程的内核栈中执行**；进程处于内核中时，其用户栈仍然保存着数据，只是暂时处于不活跃状态。进程的线程交替地使用着用户栈和内核栈。**要注意内核栈是用户代码无法使用的**，这样即使一个进程破坏了自己的用户栈，内核也能保持运行。
